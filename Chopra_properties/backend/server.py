...
DB_PATH = ROOT_DIR / "backend" / "chopra.db"
...
def listing_to_json(row: sqlite3.Row) -> dict[str, Any]:
    images = []
    try:
...
def inquiry_to_json(row: sqlite3.Row) -> dict[str, Any]:
    return {
        "id": row["id"],
        ...
        "message": row["message"],
        "createdAt": row["created_at"],
        "listingTitle": row["listing_title"] if "listing_title" in row.keys() else None,
    }
...
def list_listings():
    ...
    if search:
        where.append("(title LIKE ? OR location LIKE ? OR description LIKE ?)")
        token = f"%{search}%"
...
def increment_view(listing_id: int):
    with get_db() as conn:
        row = conn.execute("SELECT visit_count FROM listings WHERE id = ?", (listing_id,)).fetchone()
        if not row:
            return jsonify({"message": "Listing not found"}), 404
        visit_count = int(row["visit_count"]) + 1
        conn.execute("UPDATE listings SET visit_count = ?, updated_at = ? WHERE id = ?", (visit_count, now_iso(), listing_id))
        conn.commit()
...
def update_listing(listing_id: int):
    ...
    with get_db() as conn:
        ...
        if not data:
            ...
            conn.execute(f"UPDATE listings SET {columns} WHERE id = ?", params)
            row = conn.execute("SELECT * FROM listings WHERE id = ?", (listing_id,)).fetchone()
            conn.commit()
    return jsonify(listing_to_json(row))
...
def create_inquiry():
    ...
    created_at = now_iso()
    with get_db() as conn:
        listing = conn.execute("SELECT id FROM listings WHERE id = ?", (property_id,)).fetchone()
        if not listing:
            return jsonify({"message": "Listing not found"}), 404
        cur = conn.execute(
            """
